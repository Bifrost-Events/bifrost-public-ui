...
export async function assertApacheUsesQualityProfile(
  ...
  throw new Error(
    [
      ...
      '',
      'CLI (quality-db prepare) bruker .env.local-quality, men HTTP går via Apache som laster .env som standard.',
      '',
      'Løsning (velg én):',
      '  1. npm run quality:activate   (kopier .env.local-quality → .env for public-ui + backend)',
      '  2. SetEnv BIFROST_DOTENV .env.local-quality i Apache vhost for api.bifrost.local og cup-hosts',
      '  3. Restart Apache etter endring',
      '',
